	...
	private void addRooms(World world, Rect rect) {
		for (int x = rect.xMin(); x + MAX_ROOM_SIZE < rect.xMax() - 2; x += MAX_ROOM_SIZE)
			for (int y = rect.yMin(); y + MAX_ROOM_SIZE < rect.yMax() - 2; y += MAX_ROOM_SIZE) {
				final int x1 = dice.nextInt(x + 1, x + MAX_ROOM_SIZE
						- MIN_ROOM_SIZE - 1);
				final int x2 = dice.nextInt(x1 + MIN_ROOM_SIZE - 1, x
						+ MAX_ROOM_SIZE - 1);
				final int y1 = dice.nextInt(y + 1, y + MAX_ROOM_SIZE
						- MIN_ROOM_SIZE - 1);
				final int y2 = dice.nextInt(y1 + MIN_ROOM_SIZE - 1, y
						+ MAX_ROOM_SIZE - 1);
				for (int i = x1; i <= x2; i++)
					for (int j = y1; j <= y2; j++)
						world.getTile(i, j).setTile('#', Color.white, false);
			}
	}
		...
		for (int x = rect.xMin(); x < rect.xMax(); x++)
			for (int y = rect.yMin(); y < rect.yMax(); y++)
				if (dice.nextFloat() < TREE_CHANCE) {
					final Color color = dice.nextBoolean() ? TREE_COLOR1
							: TREE_COLOR2;
					world.getTile(x, y).setTile(TREE_CHAR, color, false);
				} else {
					final Color color = dice.nextBoolean() ? OPEN_COLOR1
							: OPEN_COLOR2;
					world.getTile(x, y).setTile(OPEN, color, true);
				}
		for (int x = rect.xMin(); x < rect.xMax(); x++) {
			world.getTile(x, rect.yMin()).setTile('#', OPEN_COLOR1, false);
			world.getTile(x, rect.yMax() - 1).setTile('#', OPEN_COLOR1, false);
		}
		for (int y = rect.yMin(); y < rect.yMax(); y++) {
			world.getTile(rect.xMin(), y).setTile('#', OPEN_COLOR1, false);
			world.getTile(rect.xMax() - 1, y).setTile('#', OPEN_COLOR1, false);
		}
		if (rooms)
			addRooms(world, rect);
	}
