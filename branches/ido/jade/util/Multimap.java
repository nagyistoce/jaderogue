...
public class Multimap<K, V> implements Serializable {
	...
	/**
	 * Returns a collection with the values the key is mapped to, or null if
	 * there is no mapping for the key. More formally, if this map contains a
	 * mapping from a key k to a value v such that key compares equal to k
	 * according to the map's ordering, then this method returns a collection
	 * which will contain v; otherwise it returns null.
	 * 
	 * @param key
	 *            the whose values will be returned
	 * @return a collection with the values the key is mapped to, or null if
	 ...
	 * 
	 * @param toKey
	 *            the highest key allowed in the headmap
	 * @param inclusive
	 *            if true, then the key will be allowed in the headmap
	...
	/**
	 * Returns a set of all the keys currently mapped to at least one value in
	 * the multimap.
	 * 
	 * @return a set of all the keys currently mapped to at least one value in
	 *         the multimap.
	 */
	public Set<K> keys() {
		return map.keySet();
	...
	/**
	 * Associates the value with a key, even if the key is already associated
	 * with other values.
	 * 
	 * @param key
	 *            the key which will be mapped to the value
	 * @param value
	...
	/**
	 * Removes all values associated with the key. After this function is
	 * called, if get were called using this key, null would be returned.
	 * However, if the value is associated with another key, then that mapping
	 ...
	 * one key maps to the same value, the value will be counted for each key.
	 * If this is undesirable, values().size() will not count values more than
	 * once.
	 * 
	 ...
	 * 
	 * @param fromKey
	 *            the lowest key allowed in the tailmap
	 * @param inclusive
	 *            if true, then the key will be allowed in the tailmap
	 * @return multimap with the values for every key that is greater than the
	 *         key
	 */
	public Multimap<K, V> tailMap(K fromKey, boolean inclusive) {
		final Multimap<K, V> tail = new Multimap<K, V>();
		tail.map = new TreeMap<K, Collection<V>>(map
