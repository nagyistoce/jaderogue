...
public class AStar implements Path {
	...
	protected AStar(boolean diagonals) {
	...
	private Set<Node> getAdjacentNodes(Node node, World world) {
		...
		adjacent.add(getNode(new Coord(node.coord.getX() + 1, node.coord.getY())));
		adjacent.add(getNode(new Coord(node.coord.getX() - 1, node.coord.getY())));
		adjacent.add(getNode(new Coord(node.coord.getX(), node.coord.getY() - 1)));
		adjacent.add(getNode(new Coord(node.coord.getX(), node.coord.getY() + 1)));
		if (diagonals) {
			adjacent.add(getNode(new Coord(node.coord.getX() + 1, node.coord
					.getY() - 1)));
			adjacent.add(getNode(new Coord(node.coord.getX() + 1, node.coord
					.getY() + 1)));
			adjacent.add(getNode(new Coord(node.coord.getX() - 1, node.coord
		...
		for (final Node n : adjacent)
			if (!world.isPassable(n.coord.getX(), n.coord.getY()))
				nonpassable.add(n);
		...
		while (!open.isEmpty()) {
			...
			for (final Node y : getAdjacentNodes(x, world)) {
				if (closed.contains(y))
					continue;
				final double tentativeGScore = x.gScore
						+ hEstimate(x.coord, y.coord);
	...
	/**
	 * This method is the heuristic estimate used to optimize A*. In other
	 * words, it is the likley hood that a square will be on the shortest path
	 * between two nodes. By default, it just calculates the distance from the
	 * c1 to c2, but could be overridden to factor in other cost in node
	 * traversal.
	 * 
	 * @param c1
	...
	protected double hEstimate(Coord c1, Coord c2) {
		final int a = c1.getX() - c2.getX();
		final int b = c1.getY() - c2.getY();
