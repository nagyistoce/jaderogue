...
public class Wilderness implements Gen
{
	...
	private void addRooms(World world)
	{
		for(int x = 0; x + MAX_ROOM_SIZE < world.width - 2; x += MAX_ROOM_SIZE)
			...
			{
				int x1 = dice.nextInt(x + 1, x + MAX_ROOM_SIZE - MIN_ROOM_SIZE - 1);
				int x2 = dice.nextInt(x1 + MIN_ROOM_SIZE - 1, x + MAX_ROOM_SIZE - 1);
				int y1 = dice.nextInt(y + 1, y + MAX_ROOM_SIZE - MIN_ROOM_SIZE - 1);
				int y2 = dice.nextInt(y1 + MIN_ROOM_SIZE - 1, y + MAX_ROOM_SIZE - 1);
				for(int i = x1; i <= x2; i++)
					for(int j = y1; j <= y2; j++)
