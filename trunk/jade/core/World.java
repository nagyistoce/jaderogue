...
import jade.util.Scheduler;
import jade.util.Scheduler.Filter;
...
public abstract class World extends Messenger
{
    ...
    private Scheduler<Actor> scheduler;
    ...
    public World(int width, int height)
    {
        ...
        scheduler = new Scheduler<Actor>();
    }

    /**
     * Performs the events of one game world tick. This method calls the act of each actor, in the
     * order specified by the actOrder. Any expired actors are then removed.
     */
    public void tick()
    {
        Set<Actor> actors = scheduler.next();
        for(Class<? extends Actor> cls : actOrder)
            for(Actor actor : actors)
                if(cls.isInstance(actor))
                    scheduler.schedule(actor.act(), actor);
        removeExpired();
    }
    ...
    /**
     * Returns the draw order for this World. This is the actual instance used by the World, so any
     * changes made to this list will be reflected in the look and lookAll methods.
     * @return the draw order for this World
     */
    public final List<Class<? extends Actor>> drawOrder()
    {
        return drawOrder;
    }

    /**
     * Returns the act order for this World. This the actual instance used by the World, so any
     * changes made to this list will be reflected in the tick method.
     * @return the act order for this World
     */
    public final List<Class<? extends Actor>> actOrder()
    {
        return actOrder;
    }
    ...
    /**
     * Adds an Actor to the World at a random open tile, as generated by the specified Dice with a
     * call to getOpenTile.
     * @param actor the Actor to be added
    ...
    /**
     * Adds an Actor to the World at a random open tile, as generated by the global instance of Dice
     * with a call to getOpenTile.
     * @param actor the Actor to be added
     */
    ...
    /**
     * Removes an Actor from the World. The method raises an error if the Actor is not bound to this
     * World.
    ...
    public void removeExpired()
    {
        ...
        for(Actor actor : expired)
            if(actor.bound(this))
                removeActor(actor);
        scheduler.filter(new ExpiredFilter(this));
    }

    private class ExpiredFilter implements Filter<Actor>
    {
        private World world;

        public ExpiredFilter(World world)
        {
            this.world = world;
        }

        @Override
        public boolean predicate(Actor actor)
        {
            return actor.expired() || !actor.bound(world);
        }
    }

    /**
     * Returns all instances of Actor of type T that are currently bound to this World. Note that
     * the return type is based on T, and is determined by cls.
    ...
    /**
     * Returns an arbitrary actor of type T that is currently bound to the World, or null if there
     * is none. Note that the return type is T and is determined by cls.
    ...
    /**
     * Returns all instances of Actor of type T occupying the tile at (x, y). Note that the return
     * type is based on T, and is determined by cls.
    ...
    /**
     * Returns all instances of Actor of type T occupying the tile at (x, y), which is determined by
     * the given Coord. Note that the return type is based on T, and is determined by cls.
     * @param <T> the type that filters each instance of Actor
     * @param cls the Class that determines T
    ...
    /**
     * Returns an arbitrary Actor of type T which occupies the tile at (x, y), or returns null if
     * there is none. Note that the return type is T and is determined by cls.
    ...
    /**
     * Returns an arbitrary Actor of type T which occupies the tile at (x, y), or returns null if
     * there is none, where (x, y) is determined by coord. Note that the return type is T and is
     * determined by cls.
     * @param <T> the return type
     * @param cls the Class that determines T
     * @param coord the value of the location being queried
     ...
     * highest prioirty (the tile returned by look) is first, and the lowest prioirty is last. This
    ...
    /**
     * Returns a randomly choosen open tile within the given rectangle bounds using the given
     * instance of Dice for random number generation. A tile is considered open if it is passable
     * and there is not Actor occupant.
    ...
    /**
     * Returns a randomly choosen open tile within the given rectangle bounds. A tile is considered
     * open if it is passable and there is not Actor occupant.
     * @param x1 the minimum x value of the tile
     * @param y1 the minumum y value of the tile
     * @param x2 the maximum x value of the tile
     * @param y2 the maximum y value of the tile
    ...
    /**
     * Returns the location of a randomly choosen open tile using the given instance of Dice is used
     * for random number generation. A tile is considered open if it is passable and there is not
     * Actor occupant.
     * @param dice the instance of Dice used for random number generation
    ...
    /**
     * Returns the location of a randomly choosen open tile. The global instance of Dice is used for
     * random number generation. A tile is considered open if it is passable and there is not Actor
     * occupant.
     * @return the location of a randomly choosen open tile
     */
    ...
    /**
     * Returns true if the tile at (x, y), as specified by the given Coord, is passable.
    ...
    /**
     * Sets the face and passable value of the tile at (x, y), as given by the Coord
     * @param pos the position of the location being queried
     * @param face the new face of the tile
     * @param passable new the passable value of the tile
     */
