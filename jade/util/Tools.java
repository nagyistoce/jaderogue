...
public class Tools {
	/**
	 * Translates a character to an integer where 'a' maps to 0, 'b' to 1, 'c'
	 * to 2 and so on.
	 * 
	 * @param ch
	 ...
	 * 
	 * @param field
	 *            the Collection<Coord> to be filtered
	 * @param x
	...
	public static void filterCircle(Collection<Coord> field, int x, int y,
		...
		for (final Coord coord : field)
			if (!inCircle(x, y, coord.getX(), coord.getY(), radius))
				out.add(coord);
		field.removeAll(out);
	}
	 ...
	 *            the y-coordinate of the center of the circle
	 * @param x
	 *            the x-coordinate of the Coord in question
	 * @param y
	 *            the y-coordinate of the Coord in question
	 * @param r
	 *            the radius of the circle
	 ...
	 * coordinates of the returned direction will be between -1 and 1. This
	 * coord can then be used to translate other coord in the direction of the
	 * keypress.
	 * 
	 * @param key
	 *            the character of the keypress
	 * @param vi
	 *            if true, then keyboard controls for moving the cursor in vim
	 *            will be allowed.
	 * @param numeric
	 *            if true, then keyboard controls for moving the cursor on a
	 ...
	 * @param length
	 *            the minimum length of the return value
	...
	/**
	 * Returns a string that has the specified suffix. If the input string
	 * already has the suffix, then it will be returned unchanged, otherwise,
	 * return value will be the string concatonated with the suffix.
	 * 
	 * @param str
	 *            the string in question
	 * @param suffix
	 *            the suffix to tested
